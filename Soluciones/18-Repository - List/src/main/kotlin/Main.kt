...
fun main() {
    ...
    repository.filterBy { it.edad > 30 }.forEach {
        println(it)
        println(it.nombre)
    ...
    repository.filterBy { it.nombre.startsWith("J") }.forEach {
        println(it)
    }

    println("Estudiantes con nombre que empieza por J y son mayores de 30:")
    repository.filterBy { it.nombre.startsWith("J") && it.edad > 30 }.forEach {
        println(it)
    }

    println("Estudiantes que han aprobado:")
    repository.filterBy { it.isAprobado }.forEach {
        println(it)
    }
